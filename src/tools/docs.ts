...
export function registerDocTools(server: McpServer, gql: GraphQLClient, defaults: { workspaceId?: string }) {
  ...
  async function getTrashedDocIds(gql: GraphQLClient, workspaceId: string): Promise<Set<string>> {
    try {
      ...
      // WebSocket may be unreachable (e.g. NordVPN blocking Node TCP); degrade gracefully
  ...
  function createBlock(
    ...
    switch (normalized.type) {
      ...
      case "table": {
        ...
        block.set("sys:parent", parentId);
        block.set("sys:children", new Y.Array<string>());
        const rowIdsList: string[] = [];
        const colIdsList: string[] = [];
  ...
  const listDocsHandler = async (parsed: { workspaceId?: string; first?: number; after?: string }) => {
    // If workspaceId is provided, scope to that workspace only.
    if (parsed.workspaceId) {
      const workspaceId = parsed.workspaceId;
      const listQuery = `query ListDocs($workspaceId: String!, $first: Int, $after: String){ workspace(id:$workspaceId){ docs(pagination:{first:$first, after:$after}){ totalCount pageInfo{ hasNextPage endCursor } edges{ node{ id workspaceId title summary public defaultRole createdAt updatedAt } } } } }`;
      
      const allEdges: any[] = [];
      let after = parsed.after;
      let hasNextPage = true;
      
      // If 'first' is provided, we return only one page.
      if (parsed.first) {
        const data = await gql.request<{ workspace: any }>(listQuery, { workspaceId, first: parsed.first, after });
        const page = data.workspace.docs;
        allEdges.push(...(page.edges || []));
      } else {
        // Auto-paginate all docs
        while (hasNextPage) {
          const data = await gql.request<{ workspace: any }>(listQuery, { workspaceId, first: 100, after });
          const page = data.workspace.docs;
          allEdges.push(...(page.edges || []));
      ...
      const filteredEdges = allEdges.filter((e: any) => !trashedIds.has(e.node.id));
      return text(filteredEdges.map((e: any) => ({ ...e.node, workspaceId })));
    }

    // No workspaceId: list across all accessible workspaces
    const wsQuery = `query { workspaces { id } }`;
    const wsData = await gql.request<{ workspaces: any[] }>(wsQuery);
    const workspaces = wsData.workspaces || [];
    if (workspaces.length === 0) return text([]);
    ...
    const results = await Promise.allSettled(
      workspaces.map(async (ws) => {
        ...
        let hasNextPage = true;
        while (hasNextPage) {
          const data: any = await gql.request<{ workspace: any }>(listQuery, { workspaceId: ws.id, first: 100, after });
          const page: any = data.workspace.docs;
          allEdges.push(...(page.edges || []));
          hasNextPage = page.pageInfo?.hasNextPage || false;
          after = page.pageInfo?.endCursor;
        }
        ...
        return allEdges
          .filter((e: any) => !trashedIds.has(e.node.id))
          .map((e: any) => ({ ...e.node, workspaceId: ws.id }));
      })
    );
  ...
  server.registerTool(
    "list_docs",
    {
      ...
      inputSchema: {
        workspaceId: z.string().describe("Workspace ID to scope the listing. Omit to list across all workspaces.").optional(),
        first: z.number().optional().describe("Page size for a single page (default: auto-paginate all). If set, returns only one page of this size."),
        after: z.string().optional().describe("Cursor for cursor-based pagination (used with first).")
      }
    },
  ...
  const getDocHandler = async (parsed: { workspaceId?: string; docId: string }) => {
    ...
    if (hintWorkspaceId) {
      ...
      const data = await gql.request<{ workspace: any }>(query, { workspaceId: hintWorkspaceId, docId: parsed.docId });
    ...
    if (!docData || (!docData.title && !docData.createdAt)) {
      ...
      for (const ws of otherWorkspaces) {
        const wsResult = await gql.request<{ workspace: any }>(query, { workspaceId: ws.id, docId: parsed.docId });
  ...
  server.registerTool(
    ...
    {
      title: "Get Document",
      description: "Get a document by ID (GraphQL metadata). When workspaceId is omitted, tries all workspaces (using AFFINE_WORKSPACE_ID as a hint).",
      inputSchema: {
        workspaceId: z.string().optional().describe("Workspace ID (optional hint; if omitted or not found here, searches all workspaces)."),
        docId: DocId
      }
    },
  ...
  const searchDocsHandler = async (parsed: { workspaceId?: string; keyword: string; limit?: number }) => {
    ...
    if (!parsed.workspaceId) {
      ...
      const results = await Promise.allSettled(
        workspaces.map(async (ws) => {
          const data = await gql.request<{ workspace: any }>(searchQuery, { workspaceId: ws.id, keyword: parsed.keyword, limit: parsed.limit });
          const docs = data.workspace?.searchDocs || [];
          return docs.map((d: any) => ({ ...d, workspaceId: ws.id }));
        })
      );
    ...
    try {
      ...
      if (results && results.length > 0) {
        return text(results.map((d: any) => ({ ...d, workspaceId })));
      }
    } catch (error: any) {
  ...
  server.registerTool(
    "search_docs",
    {
      ...
      inputSchema: {
        workspaceId: z.string().optional().describe("Workspace ID to scope the search. Omit to search across all workspaces."),
    ...
    const renderBlock = (blockId: string, depth: number, listIndex: number[]): void => {
      ...
      switch (flavour) {
        ...
        case "affine:table": {
          ...
          const flatCells: Record<string, Y.Text | string> = {};

          if (rowsRaw instanceof Y.Map) {
            const j = rowsRaw.toJSON() as Record<string, any>;
            for (const [k, v] of Object.entries(j)) rowsObj[k] = v;
          } else if (rowsRaw && typeof rowsRaw === "object") {
            for (const [k, v] of Object.entries(rowsRaw as Record<string, any>)) rowsObj[k] = v;
          }

          if (colsRaw instanceof Y.Map) {
            const j = colsRaw.toJSON() as Record<string, any>;
            for (const [k, v] of Object.entries(j)) colsObj[k] = v;
          } else if (colsRaw && typeof colsRaw === "object") {
            for (const [k, v] of Object.entries(colsRaw as Record<string, any>)) colsObj[k] = v;
          }

          // Scan for flat dot-notation keys: prop:rows.<id>.rowId, prop:columns.<id>.columnId, prop:cells.<key>.text
          raw.forEach((_v: any, k: string) => {
            const rowMatch = k.match(/^prop:rows\.([^.]+)\.order$/);
            if (rowMatch) { const rid = rowMatch[1]; if (!rowsObj[rid]) rowsObj[rid] = {}; rowsObj[rid].order = _v; }
            const colMatch = k.match(/^prop:columns\.([^.]+)\.order$/);
            if (colMatch) { const cid = colMatch[1]; if (!colsObj[cid]) colsObj[cid] = {}; colsObj[cid].order = _v; }
            const cellMatch = k.match(/^prop:cells\.(.+)\.text$/);
            if (cellMatch) { flatCells[cellMatch[1]] = _v; }
          });
          ...
          if (sortedRowIds.length > 0 && sortedColIds.length > 0) {
            const readCell = (rowId: string, colId: string): string => {
              ...
              if (key in flatCells) return richTextToMarkdown(flatCells[key]);
              ...
              if (cellsRaw && typeof cellsRaw === "object" && !(cellsRaw instanceof Y.Map)) {
                const c = (cellsRaw as Record<string, any>)[key];
                if (c && typeof c === "object" && "text" in c) return richTextToMarkdown(c.text);
              }
              return "";
            };
